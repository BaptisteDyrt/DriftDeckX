 ...
 */
/**
 * Détermine si l'user a touché à au moins un champ d'Identification.
 *
 * "Touché" signifie :
 *   - brandId : ≠ "" (une marque a été sélectionnée)
 *   - modelId : ≠ "" (un modèle a été sélectionné)
 *   - name    : trim() ≠ "" (au moins un caractère non-espace tapé)
 *
 * Utilisé en mode CREATE pour décider si on affiche la modale de
 * confirmation à l'annulation, ou si on sort directement (cas form vide
 * → UX plus fluide, pas de friction inutile).
 *
 * Pas utilisé en mode EDIT : le draft est pré-rempli avec les valeurs du
 * preset original, donc cette fonction renverrait quasi toujours true et
 * n'apporterait rien.
 *
 * @returns {boolean}
 */
function hasIdentificationTouched() {
  const { draft } = formState;
  if (draft.brandId && draft.brandId !== "") return true;
  if (draft.modelId && draft.modelId !== "") return true;
...
function showCancelModal(onConfirm) {
  ...
  // ── Mode CRÉATION + Identification vide → bypass de la modale
  //
  // Si l'user clique Annuler sans avoir touché à l'Identification, c'est
  // qu'il ne veut clairement pas créer de réglage → sortie directe pour
  // une UX plus fluide. Pas besoin de tester les 62 champs tuning : en
  // mode create, l'user ne peut pas y accéder tant que l'Identification
  // n'est pas remplie (bouton Suivant désactivé au step 1).
  if (!isEdit && !hasIdentificationTouched()) {
    formState = createInitialFormState();
  ...
  // ── Sinon (édition OU création avec Identification touchée) → modale
  openModal({
